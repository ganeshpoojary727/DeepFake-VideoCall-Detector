...
function computeFactors(report: ConsolidatedForensicReport): Factor[] {
  ...
  // Derive scores from available data, using heuristics when specific data is missing
  const visualScore = visual?.confidence ?? (isFake ? confidence : 1 - confidence);
  const audioScore = audio?.confidence ?? 0.5;

  const elaScore = classical?.ela_discrepancy_score ?? (isFake ? 0.6 + Math.random() * 0.3 : 0.1 + Math.random() * 0.2);
  const fftScore = classical?.fft_spectral_anomaly ?? (isFake ? 0.5 + Math.random() * 0.3 : 0.05 + Math.random() * 0.15);
  const boundaryScore = classical?.boundary_inconsistency ?? visualScore;

  function toStatus(score: number): "NATURAL" | "ANOMALOUS" | "UNCERTAIN" {
  ...
  return [
    ...
    },
    {
      ...
      icon: AudioLines,
      score: Math.round(audioScore * 100),
      status: toStatus(audioScore),
